...
//	cin >> group;
//	if (group == 1)
//		fileinput.open("1D1G input.txt");
//	else if (group == 2)
//		fileinput.open("1D2G input.txt");
//	else if (group == 7)
//		fileinput.open("1D7G input.txt");
//	else {
...
//	for (int i = 1; i <= NC; ++i)
//		h[i] = sizeC / NC; // Size of meshes of Core
//	for (int i = NC + 1; i <= NC + NR; ++i)
//		h[i] = sizeR / NR; // Size of meshes of Reflector
//
...
//		// Source vector define [high E group][low E group][mesh]
...
//			for (int i = 0; i < g; ++i)
//				for (int j = 1; j <= NC + NR; ++j)
//					phi_new[i][j] = (source[i][j] - bidiagonal[i][j] * phi_new[i][j + 1] - bidiagonal[i][j - 1] * phi_new[i][j - 1]) / diagonal[i][j];
//
//		// keff calculation
