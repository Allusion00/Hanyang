...
int main() {
	...
	while (errk > crik || errp > crip) {
		...
		for (int counts = 0; counts < 4; ++counts)
			for (int i = 1; i <= NC + NR; ++i) {
				phi1_new[i] = (source1[i] * h[i] - bidiagonal1[i] * phi1_new[i + 1] - bidiagonal1[i - 1] * phi1_new[i - 1]) / diagonal1[i];
				phi2_new[i] = (source2[i] * h[i] - bidiagonal2[i] * phi2_new[i + 1] - bidiagonal2[i - 1] * phi2_new[i - 1]) / diagonal2[i];
			}
